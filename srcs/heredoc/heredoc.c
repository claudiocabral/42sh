...
/*   Updated: 2018/06/13 11:23:29 by ccabral          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	delimit_heredoc(t_prompt **list, t_prompt **heredoc)
{
...
static void	set_quote(t_prompt *list)
{
...
int			check_heredoc(t_prompt *list)
{
	...
	while (list && list->next)
	{
		...
		set_quote(list);
		if (list->c == '<' && list->next
				&& list->next->c == '<' && get_quote(-42) == 0)
		{
			if (!(list = find_delim(list)))
				break ;
			if (delimit_heredoc(&list, &heredoc))
				return (1);
		}
